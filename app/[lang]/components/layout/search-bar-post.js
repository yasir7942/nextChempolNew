...
import { getPostBySearch } from "../../data/loader";
...
const SearchBar = () => {
  ...
  return (
    <div className="flex flex-col relative z-[100] w-full p-2 md:p-6  text-gray-800 text-center justify-center" ref={searchContainerRef}>
      ...
      <div className={`${postData.length <= 0 || !isSearchVisible ? 'hidden' : ''} w-[89%] text-left h-auto absolute top-[67px] z-40 
                ...
                <div className="flex justify-start space-x-5 items-center pl-1">
                  ...
                  <Link href={`/blog/${post.slug}`} className="flex flex-col items-start space-y-2">
                    ...
                    <div className="font-light text-xs text-black ">
                      {post.PostDate} : {post.seo?.seoDesctiption ? post.seo.seoDesctiption.split(" ").length > 25
