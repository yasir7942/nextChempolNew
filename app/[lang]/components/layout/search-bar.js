...
import { geProductsBySearch } from "../../data/loader";
  ...
  const handleSearchQuery = async (query) => {
    ...
    if (query.length > 2) {
      ...
      try {
        ...
        const result = await geProductsBySearch(locale, query);
    ...
    <div className="flex flex-col relative z-[100] w-full p-4 md:p-6 text-gray-800 text-center justify-center" ref={searchContainerRef}>
      <form className="flex item bg-center w-full gap-2 font-light  text-gray-900">
        <input
          ...
          className={`${topBar ? ' border-b border-l border-gray-300 rtl:border-l-0  rtl:border-r ' : ' border border-gray-500'} w-full px-5 py-2 text-gray-800 text-base bg-transparent outline-none   border-solid `}
        />
      ...
      left-5 bg-gray-50 backdrop-blur-md bg-opacity-80 border border-1 border-gray-500 mt-1 p-5`}>
        ...
        <Link href={`/${locale}/search?s=${(searchQuery)}`} className="w-full block h-auto mt-5 py-1 text-base bg-slate-200 
        font-normal tracking-wider text-center text-black   first-letter:uppercase">{dictionary.topbar.viewMoreSearch}</Link>
      </div>
    </div>
  );
};
