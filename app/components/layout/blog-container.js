  ...
  return (
    <div className="bg-[#F2F2F2] pt-14 w-full h-auto">
      ...
      <PaddingContainer>
        ...
        {/* <!--Latest News --> */}
        <section className="flex flex-col p-12 md:p-0   " >
              ...
              postData.data.map(post => (
                ...
                <div key={post.id} className="w-full flex flex-col text-white  md:text-left  ">
                  <a href={`/blog/${post.slug}`} >
                    ...
                    <h2 className="text-textBlue font-semibold  leading-6 text-lg md:text-base pt-3  text-justify headline">
                      {post.title}
                    </h2>
                    <p className='text-sm text-gray-800 font-light'> {moment(post.PostDate).format('MMMM D, YYYY')}</p>
                    <p className="text-lx md:text-sm text-justify text-darkGary summary">{post.seo?.seoDesctiption}</p>
                  </a>
