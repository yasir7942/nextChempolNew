...
async function fetchData(endpoint, query = "") {
    ...
    console.log("==================================================");
    console.log("[lubricant-components fetchData] URL:", url);
    ...
    try {
        ...
        console.log("[lubricant-components fetchData] status:", parsed.status);
        console.log(
            "[lubricant-components fetchData] first 1000 chars:",
            JSON.stringify(parsed.data).slice(0, 1000)
        );

        return parsed.data;
    } catch (err) {
        console.log("[lubricant-components fetchData] ERROR:", err);
        return null;
    }
}
...
async function strapiRequest(method, endpoint, payload = null) {
    ...
    console.log("==================================================");
    console.log(`[lubricant-components ${method}] URL:`, url);

    if (payload) {
        console.log(
            `[lubricant-components ${method}] payload:`,
    ...
    console.log(`[lubricant-components ${method}] status:`, parsed.status);
    console.log(
        `[lubricant-components ${method}] response:`,
...
async function getByDocumentId(endpoint, documentId, populate = {}) {
    ...
    console.log("[lubricant-components getByDocumentId] not found after all attempts:", {
        endpoint,
        documentId: wanted,
        directRes,
        docRes,
...
async function getDeleteCheck(type, documentId) {
    if (type === "product") {
        ...
        if (!productBelongsToStaticCategory(product, category)) {
            return {
                blocked: true,
                message: "Product does not belong to Lubricant Components category.",
            };
        }
    ...
    if (type === "type") {
        ...
        if (!typeBelongsToStaticCategory(item, category)) {
            return {
                blocked: true,
                message: "This Type does not belong to Lubricant Components category.",
            };
        }
...
export async function GET(req) {
    try {
        ...
        console.log("==================================================");
        console.log("[lubricant-components GET] mode:", mode);
        console.log("[lubricant-components GET] typeId:", typeId);
        console.log("[lubricant-components GET] productId:", productId);
        ...
        if (mode === "products") {
            ...
            console.log("[lubricant-components GET] category:", category);
        ...
        console.log("[lubricant-components GET ERROR]", err);
...
export async function POST(req) {
    try {
        ...
        console.log("==================================================");
        console.log("[lubricant-components POST] body:", body);
        ...
        if (type === "product") {
            ...
            /*
                Product schema uses many-to-many "types".
                Use reliable lookup because direct /products/:id can fail when id/documentId differs.
            */
            const product = await findProductForSave(productId);

            if (!product) {
                return errorJson(
                    "Product not found in published Lubricant Components category",
                    404
                );
            }
            ...
            if (!productBelongsToStaticCategory(product, category)) {
                return errorJson(
                    "Product does not belong to Lubricant Components category",
                    400
                );
            }
        ...
        console.log("[lubricant-components POST ERROR]", err);
        return errorJson(err.message || "Server error", 500);
    }
}
...
export async function DELETE(req) {
    try {
        ...
        if (type === "type") {
            ...
            if (!typeBelongsToStaticCategory(item, category)) {
                return errorJson(
                    "This Type does not belong to Lubricant Components category.",
                    400
                );
            }
        ...
        console.log("[lubricant-components DELETE ERROR]", err);
        return errorJson(err.message || "Server error", 500);
    }
}