...
async function fetchData(endpoint, query = "") {
    ...
    try {
        ...
        console.log("[lubricant-components fetchData] status:", res.status);
        console.log("[lubricant-components fetchData] first 800 chars:", text.slice(0, 800));

        try {
            return JSON.parse(text);
        } catch (error) {
            console.log("[lubricant-components fetchData] Invalid JSON:", error);
            return null;
        }
    } catch (err) {
...
async function strapiPost(endpoint, payload) {
    ...
    console.log("==================================================");
    console.log("[lubricant-components strapiPost] URL:", url);
    console.log("[lubricant-components strapiPost] payload:", JSON.stringify(payload, null, 2));
    ...
    console.log("[lubricant-components strapiPost] status:", res.status);
    console.log("[lubricant-components strapiPost] response:", JSON.stringify(data, null, 2));
...
async function strapiPut(endpoint, payload) {
    ...
    console.log("==================================================");
    console.log("[lubricant-components strapiPut] URL:", url);
    console.log("[lubricant-components strapiPut] payload:", JSON.stringify(payload, null, 2));
    ...
    console.log("[lubricant-components strapiPut] status:", res.status);
    console.log("[lubricant-components strapiPut] response:", JSON.stringify(data, null, 2));
...
export async function GET(req) {
    try {
        ...
        if (mode === "types") {
            return NextResponse.json({
                ok: true,
                items: enumItems(ENUMS.ComponentType || []),
            });
        }
        ...
        if (type === "product") {
            ...
            if (!product) {
                return NextResponse.json(
                    {
                        ok: false,
                        error: "Product not found in published Lubricant Components category",
                    },
                    { status: 404 }
                );
            }
            ...
            if (!productBelongsToStaticCategory(product)) {
                return NextResponse.json(
                    {
                        ok: false,
                        error: "Product does not belong to Lubricant Components category",
                    },
                    { status: 400 }
                );
            }
