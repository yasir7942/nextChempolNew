...
async function fetchData(endpoint, query = "") {
    ...
    try {
        ...
        console.log("[grease fetchData] status:", parsed.status);
        console.log(
            "[grease fetchData] first 1000 chars:",
...
async function strapiRequest(method, endpoint, payload = null) {
    ...
    console.log("==================================================");
    console.log(`[grease ${method}] URL:`, url);

    if (payload) {
        console.log(
            `[grease ${method}] payload:`,
    ...
    console.log(`[grease ${method}] status:`, parsed.status);
    console.log(
        `[grease ${method}] response:`,
...
async function getByDocumentId(endpoint, documentId, populate = {}) {
    ...
    console.log("[grease getByDocumentId] not found after all attempts:", {
        endpoint,
        documentId: wanted,
        directRes,
        docRes,
...
async function getDeleteCheck(type, documentId) {
    if (type === "product") {
        ...
        if (!productBelongsToStaticCategory(product, category)) {
            return {
                blocked: true,
                message: "Product does not belong to Grease Additives category.",
            };
        }
    ...
    if (type === "type") {
        ...
        if (!typeBelongsToStaticCategory(item, category)) {
            return {
                blocked: true,
                message: "This Type does not belong to Grease Additives category.",
            };
        }
...
export async function GET(req) {
    try {
        ...
        console.log("[grease GET] productId:", productId);
...
export async function POST(req) {
    try {
        ...
        if (type === "dosage") {
            ...
            if (!productBelongsToStaticCategory(product, category)) {
                return errorJson(
                    "Product does not belong to Grease Additives category",
                    400
                );
            }
        ...
        if (type === "product") {
            ...
            if (!product) {
                return errorJson(
                    "Product not found in published Grease Additives category",
                    404
                );
            }
...
export async function DELETE(req) {
    try {
        ...
        if (type === "type") {
            ...
            if (!typeBelongsToStaticCategory(item, category)) {
                return errorJson(
                    "This Type does not belong to Grease Additives category.",
                    400
                );
            }
        ...
        console.log("[grease DELETE ERROR]", err);
        return errorJson(err.message || "Server error", 500);
    }
}