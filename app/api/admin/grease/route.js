...
async function fetchData(endpoint, query = "") {
    ...
    console.log("==================================================");
    console.log("[grease fetchData] URL:", url);
    ...
    try {
        ...
        console.log("[grease fetchData] status:", res.status);
        console.log("[grease fetchData] first 800 chars:", text.slice(0, 800));

        try {
            return JSON.parse(text);
        } catch (error) {
            console.log("[grease fetchData] Invalid JSON:", error);
            return null;
        }
    } catch (err) {
        console.log("[grease fetchData] ERROR:", err);
        return null;
    }
}
...
async function strapiPost(endpoint, payload) {
    ...
    console.log("==================================================");
    console.log("[grease strapiPost] URL:", url);
    console.log("[grease strapiPost] payload:", JSON.stringify(payload, null, 2));
    ...
    console.log("[grease strapiPost] status:", res.status);
    console.log("[grease strapiPost] response:", JSON.stringify(data, null, 2));
...
async function strapiPut(endpoint, payload) {
    ...
    console.log("==================================================");
    console.log("[grease strapiPut] URL:", url);
    console.log("[grease strapiPut] payload:", JSON.stringify(payload, null, 2));
    ...
    console.log("[grease strapiPut] status:", res.status);
    console.log("[grease strapiPut] response:", JSON.stringify(data, null, 2));
...
export async function GET(req) {
    try {
        ...
        console.log("==================================================");
        console.log("[grease GET] mode:", mode);
        console.log("[grease GET] typeId:", typeId);
        ...
        if (mode === "types") {
            return NextResponse.json({
                ok: true,
                items: enumItems(ENUMS.GreaseType || []),
            });
        }
        ...
        console.log("[grease GET ERROR]", err);
...
export async function POST(req) {
    try {
        ...
        console.log("==================================================");
        console.log("[grease POST] body:", body);
        ...
        if (type === "product") {
            ...
            if (!product) {
                return NextResponse.json(
                    {
                        ok: false,
                        error: "Product not found in published Grease Additives category",
                    },
                    { status: 404 }
                );
            }
            ...
            if (!productBelongsToStaticCategory(product)) {
                return NextResponse.json(
                    {
                        ok: false,
                        error: "Product does not belong to Grease Additives category",
                    },
                    { status: 400 }
                );
            }
        ...
        console.log("[grease POST ERROR]", err);
