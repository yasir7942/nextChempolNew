...
async function fetchData(endpoint, query = "") {
    ...
    console.log("==================================================");
    console.log("[industrial fetchData] URL:", url);
    ...
    try {
        ...
        console.log("[industrial fetchData] status:", parsed.status);
        console.log(
            "[industrial fetchData] first 1000 chars:",
            JSON.stringify(parsed.data).slice(0, 1000)
        );

        return parsed.data;
    } catch (err) {
        console.log("[industrial fetchData] ERROR:", err);
        return null;
    }
}
...
async function strapiRequest(method, endpoint, payload = null) {
    ...
    console.log("==================================================");
    console.log(`[industrial ${method}] URL:`, url);

    if (payload) {
        console.log(
            `[industrial ${method}] payload:`,
    ...
    console.log(`[industrial ${method}] status:`, parsed.status);
    console.log(
        `[industrial ${method}] response:`,
...
async function getByDocumentId(endpoint, documentId, populate = {}) {
    ...
    console.log("[industrial getByDocumentId] not found after all attempts:", {
        endpoint,
        documentId: wanted,
        directRes,
        docRes,
...
async function getDeleteCheck(type, documentId) {
    if (type === "product") {
        ...
        if (!productBelongsToStaticCategory(product, category)) {
            return {
                blocked: true,
                message: "Product does not belong to Industrial Additives category.",
            };
        }
    ...
    if (type === "type") {
        ...
        if (!typeBelongsToStaticCategory(item, category)) {
            return {
                blocked: true,
                message: "This Type does not belong to Industrial Additives category.",
            };
        }
...
export async function GET(req) {
    try {
        ...
        console.log("==================================================");
        console.log("[industrial GET] mode:", mode);
        console.log("[industrial GET] typeId:", typeId);
        console.log("[industrial GET] productId:", productId);
        ...
        console.log("[industrial GET ERROR]", err);
...
export async function POST(req) {
    try {
        ...
        console.log("==================================================");
        console.log("[industrial POST] body:", body);
        ...
        if (type === "product") {
            ...
            if (!product) {
                return errorJson(
                    "Product not found in published Industrial Additives category",
                    404
                );
            }
        ...
        if (type === "oem") {
            ...
            if (!typeBelongsToStaticCategory(typeItem, category)) {
                return errorJson(
                    "Type does not belong to Industrial Additives category",
                    400
                );
            }
        ...
        console.log("[industrial POST ERROR]", err);
        return errorJson(err.message || "Server error", 500);
    }
}
...
export async function DELETE(req) {
    try {
        ...
        if (type === "type") {
            ...
            if (!typeBelongsToStaticCategory(item, category)) {
                return errorJson(
                    "This Type does not belong to Industrial Additives category.",
                    400
                );
            }
        ...
        console.log("[industrial DELETE ERROR]", err);
        return errorJson(err.message || "Server error", 500);
    }
}