...
const API_ROUTE = "/api/admin/viscosity";
...
export default function ViscosityIndexImproverPage() {
    ...
    const [allSsis, setAllSsis] = useState([]);

    const [selectedType, setSelectedType] = useState("");
    const [selectedProduct, setSelectedProduct] = useState("");
    const [selectedSsi, setSelectedSsi] = useState("");
    ...
    const selectedSsiObj = useMemo(() => {
        return findSelected(ssis, selectedSsi);
    }, [ssis, selectedSsi]);
    ...
    function showError(error) {
        console.log("[viscosity page] error:", error);
    ...
    async function fetchJson(url, options = {}) {
        ...
        try {
            ...
            console.log("[viscosity page] JSON parse error:", error);
            console.log("[viscosity page] response text:", text);
    ...
    async function loadSsis(typeId, productId) {
        if (!typeId || !productId) {
            setSsis([]);
            setAllSsis([]);
            return;
        ...
        const [data, all] = await Promise.all([
            fetchJson(apiUrl("ssis", { typeId, productId })),
            fetchJson(apiUrl("all-ssis", { typeId, productId })),
        ]);

        setSsis(data?.items || []);
        setAllSsis(all?.items || []);
    }
            ...
            try {
                ...
                if (selectedType && selectedProduct) {
                    await Promise.all([
                        ...
                        loadSsis(selectedType, selectedProduct),
                    ]);
                }
            } catch (error) {
                showError(error);
    ...
    async function handleAdd(type) {
        try {
            ...
            if (type === "ssi" && (!selectedType || !selectedProduct)) {
            ...
            if (type === "ssi") {
                ...
                if (data?.item?.documentId) {
                    setSelectedSsi(String(data.item.documentId));
                }

                showSuccess("SSI added successfully");
            }
        } catch (error) {
            showError(error);
        } finally {
    ...
    async function handleDelete(type, item) {
        try {
            ...
            const confirmText =
                type === "product"
                    ? `Remove Type / SSI relation from Product?\n\nProduct will NOT be deleted from Product collection.\nOnly relations will be removed from this Product.\n\nProduct: ${label}`
            ...
            if (type === "ssi") {
                if (String(selectedSsi) === String(documentId)) {
                    setSelectedSsi("");
                }

                if (selectedType && selectedProduct) {
                    await loadSsis(selectedType, selectedProduct);
                }

                showSuccess("SSI deleted successfully");
            }
        } catch (error) {
            showError(error);
        } finally {
                    ...
                    <div>
                        ...
                        {type === "product" && (
                            <p className="text-[11px] text-gray-500">
                                Add/select Product first, then Dosage / SSI will open.
                            </p>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_320px]">
                        ...
                        <aside className="h-fit rounded-xl border border-gray-200 bg-white p-3 shadow-sm lg:sticky lg:top-4">
                            ...
                            <div className="mt-3 space-y-2">
                                ...
                                <SummaryRow
                                    label="SSI"
                                    value={selectedSsiObj?.label}
                                />
                            </div>

                            <div className="mt-3 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-xs text-blue-700">
                                Product × only removes Type / SSI relation from Product. It does not delete Product collection record.
                            </div>
                        </aside>
